      ...
      <div className="container mx-auto px-4">
        ...
        <div className="mb-8 sm:mb-12">
          <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
            {categories.map(category => <Button key={category} variant={selectedCategory === category ? "default" : "outline"} size="sm" onClick={() => setSelectedCategory(category)} className={`${selectedCategory === category ? "gold-gradient text-white" : "border-gold text-gold hover:bg-gold hover:text-white"} transition-all duration-200`}>
                {category}
              </Button>)}
          </div>
        </div>
