...
export function JobChecklist({ projectId }: JobChecklistProps) {
  const queryClient = useQueryClient();
  const [newItem, setNewItem] = useState("");

  const { data: tasks = [], isLoading } = useQuery({
    queryKey: ["job-checklist", projectId],
    queryFn: async () => {
  ...
  const createTask = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["job-checklist", projectId] });
      setNewItem("");
    },
  });
