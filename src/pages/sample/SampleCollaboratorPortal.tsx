    ...
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="px-4 py-4 space-y-4 max-w-xl mx-auto">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {WEEK.map((d) => (
            <div
              key={d.day}
              className={`flex flex-col items-center min-w-[52px] rounded-xl py-2 px-2 border ${
                d.today ? "bg-primary text-primary-foreground border-primary" : "bg-card text-foreground border-border"
              }`}
            >
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-card border-t flex items-center justify-around py-2 max-w-xl mx-auto z-20">
        {[
          ...
          <div key={label} className={`flex flex-col items-center gap-0.5 px-3 py-1 ${active ? "text-primary" : "text-muted-foreground"}`}>
