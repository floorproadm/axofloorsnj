...
-- Create audit trigger for project data access
CREATE TRIGGER log_project_access
  AFTER SELECT ON public.projects
  FOR EACH ROW
  EXECUTE FUNCTION log_admin_access('projects', 'SELECT');
...
BEGIN
  ...
  IF TG_OP = 'SELECT' AND get_current_user_role() = 'admin' THEN
    PERFORM log_admin_access('projects', 'customer_data_access', auth.uid());
  END IF;
