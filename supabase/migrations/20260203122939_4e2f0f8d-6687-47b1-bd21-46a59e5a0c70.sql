-- Function to validate lead pipeline transitions
-- Pipeline: new → contacted → quoted → won/lost
-- Rules: 
--   1. Cannot skip stages
--   2. To move to "quoted": must have proposal with valid margin
...
BEGIN
  ...
  SELECT l.status, l.converted_to_project_id 
  ...
  -- Special case: won and lost are both terminal states from quoted
  IF p_new_status IN ('won', 'lost') THEN
    ...
    RETURN QUERY SELECT v_can_transition, v_error_message, v_current_status, v_required_status;
    RETURN;
  END IF;
...
GRANT EXECUTE ON FUNCTION public.validate_lead_transition(UUID, TEXT) TO authenticated;